...
\paragraph{}
Deux formalismes sont pris en charge: les automates et les réseaux de Petri, à la fois avec des extensions. La plupart 
des outils sont liés à des réseaux de Petri. Certains d'entre eux procèdent à des analyses structurelles comme des calculs
d'invariants. Tandis que d'autres outils effectuent des analyses comportementales : construction de graphe d'accessibilité, 
etc ...

\paragraph{}
Cet environnement se compose actuellement de deux outils logiciels : Coloane, l'interface graphique, et Alligator, une 
intégration d'outils basé sur les web services. Il est agrandie par les outils existants de vérification développés dans les laboratoires (membres ou partenaires fondateurs).
...
\paragraph{}
Le développement de CosyVerif a été décidé et soutenu par trois partenaires  du groupe de vérification parisien,
MeFoSyLoMa (Méthodes formelles pour les systèmes logiciels et matériels).

\paragraph{}
Ce groupe est composé de sept équipes, et les membres fondateurs de CosyVerif sont LIP6 (Laboratoire d'Informatique de
Paris 6) , LIPN (Laboratoire d'Informatique de Paris Nord) et LSV (Laboratoire Spécification et Vérification - Laboratoire 
d'informatique de l'ENS de Cachan).
...
\paragraph{}
Le développement de la nouvelle plate-forme CosyVerif qui représente actuellement du serveur et d'un client web.
...
\paragraph{}
L'objectif de ce stage pour ma part est donc de participer à la conception et au développement du nouveau serveur et du 
...
\begin{itemize}
\item Les tests doivent être effectuées en boîte noire avec une convertir complète du code,
\item Le code doit être maintenable et simple,
\item Le code source doit être commenté en anglais,
\item Des technologies qui présentent une surcouche et un couplage forte seront évitées,
\end{itemize}
...
\paragraph{}
Mon stage consiste donc à concevoir et à réaliser le serveur de dépôt et l'interface web du client. Les serveurs d'édition et d'exécution sont réalisés par Alban Linard (Notre encadrent) et l'éditeur web par Francisco Gimenez (Un autre stagiaire argentin).
...
à ce besoin de protection, le serveur met en place une politique de sécurité basée sur un système d'authentification et un système d'authorisation des utilisateurs mais aussi un système de chiffrement des données échangées entre les clients et le serveur.
...
éventuels hackers qui récupéreraient cette requête sur internet.
...
\begin{itemize}
   ...
   \item PUT : créer/mettre à jour une ressource,
   \item PATCH : mettre à jour une ressource partiellement,
...
\paragraph{Slim :}
est un micro framework PHP qui vous permet d'écrire rapidement des applications Web et des APIs de façon simple 
...
\paragraph{}
Simplifie le processus de construction d'un Phar
...
fichiers. Donc, le serveur va utiliser le système de fichier local.
...
avec la description de l'exception qui va pourvoir intervenir.
...
utilisateur est soumis à l'authentication.
...
à cette couche et qu'il y a lieu de traduction, il traite la traduction.

\subsubsection{Module Core}

\paragraph{}
Le module Core gère l'accès et la mise à jour des ressources.
...
PHPUnit, Guzzle et Travis sont utilisés pour faire le test. Ces tests vont nous permettre de s'assurer que le serveur 
fonctionne correctement, fonctionnellement.
